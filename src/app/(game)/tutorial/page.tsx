...
export default function Tutorial() {
        ...
        setHints([
            ...
            <div className="flex flex-col items-center space-y-4" key={"game tutorial"}>
                ...
                <div className="flex flex-col items-center">
                    ...
                    <div className="flex rounded-md overflow-hidden">
                        <Colorpad color={"Amarelo"} />
                        <Colorpad color={"Laranja"} />
                        <Colorpad color={"Verde"} />
                    </div>
                </div>
            </div>,
            ...
            <div className="flex flex-col space-y-4 text-center" key={"timer tutorial"}>
                ...
                <div className="flex items-center space-x-2 justify-center animate-pulse">
                    <TimerIcon size={90} className="text-purple-500" />
                    <p className="text-7xl text-purple-500">{Math.abs(2.00).toFixed(1)}</p>
                </div>
    ...
    return (
        <div className="flex flex-col h-screen w-full items-center p-10">
            <h1 className="text-3xl">{page + 1} / {hints.length}</h1>
            <div className="flex h-full w-full  justify-between items-center p-4">
                <ArrowLeft size={50} onClick={() => setPage(page < 1 ? 0 : page - 1)} className="hover:cursor-pointer" />
                <div className="flex flex-col p-5 space-y-3 items-center">
                    {hints[page]}
                </div>
                <ArrowRight size={50} onClick={() => setPage(page >= hints.length - 1 ? page : page + 1)} className="hover:cursor-pointer" />
            </div>
        </div>
    )
}